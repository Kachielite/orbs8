...
export class AccountService {
  ...
  async findOne(id: number, user: Partial<User>) {
    ...
    try {
      ...
      logger.error(
        `Error fetching account with ID: ${id} for user: ${user.id}: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw new InternalServerErrorException(
        `Error fetching account with ID: ${id} for user: ${user.id}`,
      );
    }
  ...
  async accountSummary(user: Partial<User>): Promise<AccountSummaryDto> {
    try {
      ...
      for (const code of uniqueCurrencies) {
        quotes[`${preferredCurrency}${code}`] = await this.exchangeRateService.getRate(
          code,
          preferredCurrency,
        );
      }
  ...
  // Exchange rate logic is handled by ExchangeRateService which runs scheduled updates.
